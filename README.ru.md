...
| `-OnlyOnChange` | — | выкл | прятаться, появляться на мгновение при смене раскладки |
| `-ShowMs` | мс | `1200` | сколько держать панель в этом режиме |
| `-Switcher` | — | выкл | показывать все установленные раскладки в ряд с едущим выделением, как HUD выбора источника ввода в macOS |
| `-Glass` | — | выкл | акрил Windows 11 вместо плотной тёмной плашки |
| `-Interval` | мс | `120` | период опроса (`15` при включённом `-OnlyOnChange`) |

Маковский вариант — `-OnlyOnChange -Switcher`: на экране пусто, пока вы
печатаете, а в момент переключения появляется панель, где выделение переезжает
на новую раскладку. В покое это почти ничего не стоит: UI Automation
дёргается только при реальной смене раскладки.
