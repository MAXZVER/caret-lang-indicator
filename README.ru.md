...
Скачайте `CaretLangIndicator.exe` и `install.ps1` из
[последнего релиза](../../releases/latest) и запустите:

```powershell
.\install.ps1
```

Скрипт копирует exe в `%LOCALAPPDATA%\CaretLangIndicator`, прописывает его в
автозагрузку и запускает. Права администратора не нужны, службы не создаются,
за пределы вашего профиля ничего не пишется.

```powershell
.\install.ps1 -Arguments '-OnlyWhenCaps'   # свои параметры
.\install.ps1 -NoAutostart                 # без автозагрузки
.\uninstall.ps1                            # удалить всё
```
